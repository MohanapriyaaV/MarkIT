...
class _CreateTeamPageState extends State<CreateTeamPage> {
  ...
  Future<void> _fetchUserRole() async {
    ...
    if (currentUser != null) {
      final doc = await FirebaseFirestore.instance.collection('employeeInfo').doc(currentUser.uid).get();
      if (doc.exists) {
        setState(() {
  ...
  Future<void> _submit() async {
    ...
    if (!isSuperAdmin) {
      hasRequiredOwners = _teamType == 'General Team'
          ? (_generalProjectManagerId != null && _assistantManagerHRId != null && _managerHRId != null)
          : (_projectManagerId != null && _assistantProjectManagerId != null && _projectLeadId != null);
    }
    ...
    if (_formKey.currentState!.validate() &&
      ...
      try {
        ...
        final session1Login = DateTime(now.year, now.month, now.day, _session1Login!.hour, _session1Login!.minute);
        final session1Logout = DateTime(now.year, now.month, now.day, _session1Logout!.hour, _session1Logout!.minute);
        final session2Login = DateTime(now.year, now.month, now.day, _session2Login!.hour, _session2Login!.minute);
        final session2Logout = DateTime(now.year, now.month, now.day, _session2Logout!.hour, _session2Logout!.minute);
        ...
        if (isEditMode) {
          await FirebaseFirestore.instance.collection('teams').doc(widget.editTeam!.teamId).update(teamData);
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Team updated successfully!')));
        } else {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to ${isEditMode ? 'update' : 'create'} team: ${e.toString()}')),
        );
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Please fill all required fields and assign appropriate roles.')),
      );
    }
  }
            ...
            : Form(
                ...
                child: ListView(
                  children: [
                    ...
                    SizedBox(height: 16),
                    Text("Shift Timings:", style: TextStyle(fontWeight: FontWeight.bold)),
                    _buildTimePicker("Session 1 Login", _session1Login, (val) => _session1Login = val),
                    _buildTimePicker("Session 1 Logout", _session1Logout, (val) => _session1Logout = val),
                    _buildTimePicker("Session 2 Login", _session2Login, (val) => _session2Login = val),
                    _buildTimePicker("Session 2 Logout", _session2Logout, (val) => _session2Logout = val),
                    SizedBox(height: 16),
                    TextFormField(
                      controller: _graceTimeController,
                      decoration: InputDecoration(labelText: 'Grace Time (in minutes)'),
                    ...
                    TextFormField(
                      ...
                      decoration: InputDecoration(labelText: 'Emergency Leaves'),
                      keyboardType: TextInputType.number,
                      validator: (value) => value!.isEmpty ? 'Enter number' : null,
                    ),
                    SizedBox(height: 16),
                    ElevatedButton(
                      ...
                      child: Text('Select Members (${_selectedMembers.length})'),
                    ),
                    SizedBox(height: 20),
                    ElevatedButton(
                      onPressed: _submit,
